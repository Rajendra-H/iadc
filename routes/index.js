...
router.post('/book', function(req, res) {
    ...
    // var sql = "SELECT * FROM booking_details WHERE hall_name = ? AND date = ? AND (fromtime = ? AND totime = ?) OR (fromtime <= ? AND totime > ?) OR (fromtime < ? AND totime >= ?)"
    ...
    // connection.query(sql,[hallname,date], function (err, result) {
        if (err) throw err;
        if(result.length === 0){
